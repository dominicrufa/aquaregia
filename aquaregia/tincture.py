...
def get_pull_samples_fn(p_i_samples, p_f_samples, minibatch_size, ke_sampler):
...
def get_RNVP_trainer(u_i,
    ...
    initd_forward_rnvp = forward_rnvp()
    nn_params = initd_forward_rnvp.init(init_fwd_seed, init_xs, init_vs)
    in_forward_work_fn = partial(forward_work_fn, u_i_fn = u_i, u_f_fn = u_f, ke_fn = ke_fn, initd_forward_rnvp = initd_forward_rnvp, kT=kT)

    if backward_rnvp is not None and p_f_train_samples is not None:
        bidirectional = True
